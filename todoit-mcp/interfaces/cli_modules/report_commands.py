...
@click.option('--filter', 'list_filter', 
    ...
    try:
        ...
        if list_filter:
            try:
                ...
                console.print(f"[dim]  ^\\d{{4}}_.*     - Lists starting with 4 digits + underscore[/]")
        ...
        if not failed_items:
            if list_filter:
                console.print(f"[yellow]No failed tasks found in active lists matching pattern: {list_filter}[/]")
            else:
        ...
        for item_info in failed_items:
            ...
            if item_info.get('properties'):
                props_str = ", ".join([f"{k}={v}" for k, v in item_info['properties'].items()])
                props_display = props_str[:30] + "..." if len(props_str) > 30 else props_str
            else:
                props_display = "-"
        ...
        if _get_output_format() in ['table', 'vertical']:
            ...
            if has_properties:
                console.print(f"[dim]💡 Some tasks have additional properties - use JSON output for full details[/]")
