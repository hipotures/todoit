...
def create_tag(ctx, name, color):
    ...
    try:
        ...
        console.print(
            f"✅ Tag '{tag.name}' created successfully with color '{tag.color}'"
        )
...
def list_tags(ctx):
    ...
    try:
        ...
        for tag in tags:
            ...
            data.append(
                {"Name": tag.name, "Color": color_display, "Created": created_date}
            )
...
def delete_tag(ctx, name, force):
    ...
    try:
        ...
        if not force:
            ...
            if not Confirm.ask(
                f"❓ Delete tag '{tag.name}'? This will remove it from all lists"
            ):
