...
def _render_tree_view(todo_list, items: List, properties: Dict[str, str], manager = None) -> Tree:
...
def _render_table_view(todo_list, items: List, properties: Dict[str, str], manager = None):
...
def _create_list_info_panel(todo_list, progress, last_update_time, has_changed, no_heartbeat=False):
    ...
    info_text.append(f"Progress: {progress.completion_percentage:.1f}%\n", style="yellow")
...
def _create_changes_panel(changes_history):
    ...
    for i, change in enumerate(changes_history[-5:], 1):  # Last 5 changes
        ...
        changes_text.append(f"{change.get('description', 'Unknown change')}\n", style="white")
