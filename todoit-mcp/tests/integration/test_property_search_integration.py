...
class TestPropertySearchIntegration:
    ...
    def setup_test_data(self, manager):
        ...
        # Add test items
        item1 = manager.add_item("testlist", "task1", "First task", metadata={"priority": "high"})
        item2 = manager.add_item("testlist", "task2", "Second task", metadata={"priority": "low"})
        item3 = manager.add_item("testlist", "task3", "Third task", metadata={"status": "reviewed"})
    ...
    def test_database_find_items_by_property_integration(self, manager, setup_test_data):
    ...
    def test_database_find_items_by_property_with_limit(self, manager, setup_test_data):
        ...
        # Search with limit=1
        results = manager.db.find_items_by_property(db_list.id, "category", "bug", limit=1)
    ...
    def test_manager_find_items_by_property_multiple_results(self, manager, setup_test_data):
    ...
    @pytest.mark.asyncio
    async def test_mcp_find_items_by_property_integration(self, manager, setup_test_data):
    ...
    def test_cli_item_find_integration(self, manager, setup_test_data):
        ...
        result = runner.invoke(item_find, [
            "testlist", 
            "--property", "category", 
            "--value", "bug"
    ...
    def test_cli_item_find_no_results(self, manager, setup_test_data):
        ...
        assert ("Found 0 item(s)" in output) or ("No" in output and "search results" in output)
