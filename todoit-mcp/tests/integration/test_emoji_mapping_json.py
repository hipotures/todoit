...
class TestEmojiMappingJSON:
    ...
    def test_list_all_json_emoji_mapping(self, runner):
        ...
        with runner.isolated_filesystem():
            ...
            result = runner.invoke(cli, ['--db', 'test.db', 'list', 'create', 'testlist', '--title', 'Test List'])
            assert result.exit_code == 0
            
            result = runner.invoke(cli, ['--db', 'test.db', 'item', 'add', 'testlist', 'task1', 'Task 1'])
            assert result.exit_code == 0
            
            result = runner.invoke(cli, ['--db', 'test.db', 'item', 'add', 'testlist', 'task2', 'Task 2'])
            assert result.exit_code == 0
            
            # Mark one task as completed
            result = runner.invoke(cli, ['--db', 'test.db', 'item', 'status', 'testlist', 'task1', '--status', 'completed'])
            assert result.exit_code == 0
            
            # Test JSON output
            result = runner.invoke(cli, ['--db', 'test.db', 'list', 'all'], env={'TODOIT_OUTPUT_FORMAT': 'json'})
            assert result.exit_code == 0
    ...
    def test_yaml_output_emoji_mapping(self, runner):
        ...
        with runner.isolated_filesystem():
            ...
            # Test YAML output
            result = runner.invoke(cli, ['--db', 'test.db', 'list', 'all'], env={'TODOIT_OUTPUT_FORMAT': 'yaml'})
            assert result.exit_code == 0
    ...
    def test_xml_output_emoji_mapping(self, runner):
        ...
        with runner.isolated_filesystem():
            ...
            # Test XML output
            result = runner.invoke(cli, ['--db', 'test.db', 'list', 'all'], env={'TODOIT_OUTPUT_FORMAT': 'xml'})
            assert result.exit_code == 0
    ...
    def test_mapping_completeness(self):
        ...
        for emoji, name in EMOJI_TO_NAME_MAPPING.items():
            ...
            assert name.replace('_', '').isalnum(), f"Name '{name}' for {emoji} should be alphanumeric with underscores"
            assert not name.startswith('_'), f"Name '{name}' for {emoji} should not start with underscore"
    ...
    def test_multiple_lists_json_mapping(self, runner):
        ...
        with runner.isolated_filesystem():
            ...
            result = runner.invoke(cli, ['--db', 'test.db', 'list', 'create', 'list1', '--title', 'List 1'])
            assert result.exit_code == 0
            
            result = runner.invoke(cli, ['--db', 'test.db', 'list', 'create', 'list2', '--title', 'List 2'])
            assert result.exit_code == 0
            
            # Add items to first list
            result = runner.invoke(cli, ['--db', 'test.db', 'item', 'add', 'list1', 'task1', 'Task 1'])
            assert result.exit_code == 0
            
            result = runner.invoke(cli, ['--db', 'test.db', 'item', 'add', 'list1', 'task2', 'Task 2'])
            assert result.exit_code == 0
            
            # Mark one as failed, one as completed
            result = runner.invoke(cli, ['--db', 'test.db', 'item', 'status', 'list1', 'task1', '--status', 'completed'])
            assert result.exit_code == 0
            
            result = runner.invoke(cli, ['--db', 'test.db', 'item', 'status', 'list1', 'task2', '--status', 'failed'])
            assert result.exit_code == 0
