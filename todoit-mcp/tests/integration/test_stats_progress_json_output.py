...
class TestStatsProgressJsonOutput:
    ...
    def test_stats_progress_json_output_with_mixed_tasks(self):
        ...
        with self.runner.isolated_filesystem():
            # Create a test list
            result = self.runner.invoke(cli, ['--db', 'test.db', 'list', 'create', 'testlist', '--title', 'Progress Test List'])
            ...
            result = self.runner.invoke(cli, ['--db', 'test.db', 'item', 'add', 'testlist', 'task2', 'In Progress Task'])
            assert result.exit_code == 0
            
            result = self.runner.invoke(cli, ['--db', 'test.db', 'item', 'add', 'testlist', 'task3', 'Pending Task'])
            assert result.exit_code == 0
            
            result = self.runner.invoke(cli, ['--db', 'test.db', 'item', 'add', 'testlist', 'task4', 'Failed Task'])
            assert result.exit_code == 0
            ...
            result = self.runner.invoke(cli, ['--db', 'test.db', 'item', 'status', 'testlist', 'task4', '--status', 'failed'])
            assert result.exit_code == 0
            
            # Test JSON output for progress
            result = self.runner.invoke(cli, ['--db', 'test.db', 'stats', 'progress', 'testlist'])
            assert result.exit_code == 0
    ...
    def test_stats_progress_json_output_empty_list(self):
        ...
        with self.runner.isolated_filesystem():
            ...
            result = self.runner.invoke(cli, ['--db', 'test.db', 'stats', 'progress', 'emptylist'])
            assert result.exit_code == 0
    ...
    def test_stats_progress_json_output_all_completed(self):
        ...
        with self.runner.isolated_filesystem():
            # Create a test list
            result = self.runner.invoke(cli, ['--db', 'test.db', 'list', 'create', 'testlist', '--title', 'Completed List'])
            assert result.exit_code == 0
            
            # Add tasks and complete them
            result = self.runner.invoke(cli, ['--db', 'test.db', 'item', 'add', 'testlist', 'task1', 'Task 1'])
            assert result.exit_code == 0
            
            result = self.runner.invoke(cli, ['--db', 'test.db', 'item', 'add', 'testlist', 'task2', 'Task 2'])
            assert result.exit_code == 0
            
            result = self.runner.invoke(cli, ['--db', 'test.db', 'item', 'status', 'testlist', 'task1', '--status', 'completed'])
            assert result.exit_code == 0
            
            result = self.runner.invoke(cli, ['--db', 'test.db', 'item', 'status', 'testlist', 'task2', '--status', 'completed'])
            assert result.exit_code == 0
    ...
    def test_stats_progress_json_output_with_detailed_flag(self):
        ...
        with self.runner.isolated_filesystem():
            ...
            result = self.runner.invoke(cli, ['--db', 'test.db', 'stats', 'progress', 'testlist', '--detailed'])
            assert result.exit_code == 0
