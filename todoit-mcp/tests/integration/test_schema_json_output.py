...
class TestSchemaJsonOutput:
    ...
    def test_schema_json_output_categories(self):
        ...
        with self.runner.isolated_filesystem():
            ...
            categories = set(item['Category'] for item in output_data['data'] if item['Category'])
    ...
    def test_schema_json_output_item_statuses(self):
        ...
        with self.runner.isolated_filesystem():
            ...
            status_items = [item for item in output_data['data'] 
                          if item['Category'] == 'Item Statuses' or 
                          (item['Category'] == '' and item['Value'] in ['pending', 'in_progress', 'completed', 'failed'])]
            ...
            status_with_descriptions = [item for item in status_items if item['Description']]
    ...
    def test_schema_json_output_dependency_types(self):
        ...
        with self.runner.isolated_filesystem():
            ...
            dep_items = [item for item in output_data['data'] 
                        if item['Category'] == 'Dependency Types' or 
                        (item['Category'] == '' and item['Value'] in ['blocks', 'requires', 'related'])]
    ...
    def test_schema_json_output_list_types(self):
        ...
        with self.runner.isolated_filesystem():
            ...
            list_items = [item for item in output_data['data'] 
                         if item['Category'] == 'List Types' or 
                         (item['Category'] == '' and item['Value'] in ['sequential', 'parallel'])]
    ...
    def test_schema_json_output_descriptions_present(self):
        ...
        with self.runner.isolated_filesystem():
            ...
            items_with_descriptions = [item for item in output_data['data'] if item['Description']]
            items_without_descriptions = [item for item in output_data['data'] if not item['Description']]
            ...
            pending_item = next((item for item in output_data['data'] if item['Value'] == 'pending'), None)
    ...
    def test_schema_json_count_accuracy(self):
        ...
        with self.runner.isolated_filesystem():
            ...
            assert output_data['count'] >= 15  # We have 5 categories with multiple values each
    ...
    def test_schema_json_category_grouping(self):
        ...
        with self.runner.isolated_filesystem():
            ...
            empty_category_items = [item for item in output_data['data'] if not item['Category']]
            assert len(empty_category_items) > 0  # Should have items grouped under categories