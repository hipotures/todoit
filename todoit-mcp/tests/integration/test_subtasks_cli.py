...
class TestSubtasksCLI:
    ...
    def test_cli_create_list_with_subtasks(self, temp_db_path):
        ...
        # Add subtask
        result = self.run_cli(f"item add-subtask test_list main_task sub1 'Subtask 1'", temp_db_path)
    ...
    def test_cli_subtask_status_update(self, temp_db_path):
        ...
        # Update status
        result = self.run_cli(f"item status test_list main_task completed", temp_db_path)
    ...
    def test_cli_move_task_to_subtask(self, temp_db_path):
        ...
        result = self.run_cli(f"item move-to-subtask test_list task2 task1", temp_db_path)
        # CLI command may not exist - test documents this
        assert result.returncode == 0 or "move-to-subtask" in result.stderr or "not found" in result.stderr
    ...
    def test_cli_item_hierarchy_display(self, temp_db_path):
        ...
        assert result.returncode == 0 or "tree" in result.stderr or "not found" in result.stderr
