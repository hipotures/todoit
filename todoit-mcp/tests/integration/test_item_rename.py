...
class TestItemRename:
    ...
    def test_manager_rename_item_key_only(self, temp_db_path, setup_test_data):
        ...
        updated_item = manager.rename_item(
            list_key="project", item_key="task1", new_key="renamed_task"
        )
    ...
    def test_manager_rename_item_title_only(self, temp_db_path, setup_test_data):
        ...
        updated_item = manager.rename_item(
            list_key="project", item_key="task1", new_content="Updated Task Title"
        )
    ...
    def test_manager_rename_item_both_key_and_title(
    ...
    def test_manager_rename_item_validation_errors(self, temp_db_path, setup_test_data):
        ...
        with pytest.raises(ValueError, match="already exists"):
            manager.rename_item(
                "project", "task1", new_key="task2"
            )  # task2 already exists
    ...
    def test_cli_item_rename_error_no_parameters(self, temp_db_path, setup_test_data):
        ...
        assert (
            "At least one of --new-key or --new-title must be provided" in result.output
        )
    ...
    def test_rename_item_with_dependencies_preserved(
        self, temp_db_path, setup_test_data
    ):
        ...
        other_item = manager.add_item(
            "other_project", "dependent_task", "Dependent Task"
        )
        ...
        manager.rename_item(
            list_key="project", item_key="task1", new_key="renamed_required"
        )
