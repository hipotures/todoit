...
class TestManagerComprehensive:
    ...
    def test_item_management_comprehensive(self, temp_manager):
        ...
        item2 = temp_manager.add_item(
            "item_test", "item2", "Item 2 Content", metadata={"priority": "normal"}
        )
    ...
    def test_bulk_operations_comprehensive(self, temp_manager):
        ...
        category_a_items = [
            item for item in all_items if item.metadata.get("category") == "A"
        ]

        for item in category_a_items:
            temp_manager.update_item_status(
                "bulk_test", item.item_key, status="in_progress"
            )
        ...
        for item in updated_items:
            temp_manager.update_item_status(
                "bulk_test", item.item_key, status="completed"
            )
    ...
    def test_search_and_filtering(self, temp_manager):
        ...
        high_priority = [
            item for item in all_items if item.metadata.get("priority") == "high"
        ]
        assert len(high_priority) == 2

        # Filter by metadata category
        backend_tasks = [
            item for item in all_items if item.metadata.get("category") == "backend"
        ]
        ...
        api_tagged = [
            item
            for item in all_items
            if "tags" in item.metadata and "api" in item.metadata["tags"]
        ]
...
"""
        ...
        try:
            ...
            imported_lists = temp_manager.import_from_markdown(
                markdown_file, base_key="imported_test"
            )
