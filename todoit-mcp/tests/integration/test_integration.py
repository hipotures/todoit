...
def test_list_create_and_show(temp_db_path):
    ...
    runner = CliRunner()
    # Create a list
    result = runner.invoke(cli, ['--db', temp_db_path, 'list', 'create', 'test-list', '--title', 'My Test List'])
    assert result.exit_code == 0
    assert "List Created" in result.output

    # Show the list
    result = runner.invoke(cli, ['--db', temp_db_path, 'list', 'show', 'test-list', '--tree'])
    assert result.exit_code == 0
...
def test_item_add_and_list(temp_db_path):
    ...
    # Add an item
    result = runner.invoke(cli, ['--db', temp_db_path, 'item', 'add', 'item-list', 'test-item', 'My first task'])
    assert result.exit_code == 0
    assert "Added item" in result.output

    # Show the list and check for the item
    result = runner.invoke(cli, ['--db', temp_db_path, 'list', 'show', 'item-list', '--tree'])
    assert result.exit_code == 0
...
def test_item_status_update(temp_db_path):
    ...
    runner.invoke(cli, ['--db', temp_db_path, 'item', 'add', 'status-list', 'task1', 'Task to update'])

    # Update status to 'in_progress'
    result = runner.invoke(cli, ['--db', temp_db_path, 'item', 'status', 'status-list', 'task1', '--status', 'in_progress'])
    assert result.exit_code == 0
    assert "Updated 'task1'" in result.output

    # Verify the status change
    result = runner.invoke(cli, ['--db', temp_db_path, 'list', 'show', 'status-list', '--tree'])
...
def test_subtask_creation_and_hierarchy(temp_db_path):
    ...
    runner.invoke(cli, ['--db', temp_db_path, 'item', 'add', 'hier-list', 'parent', 'Parent Task'])
    
    # Add a subtask
    result = runner.invoke(cli, ['--db', temp_db_path, 'item', 'add-subtask', 'hier-list', 'parent', 'child', 'Child Task'])
    assert result.exit_code == 0
    assert "Added subtask" in result.output

    # Check the tree view
    result = runner.invoke(cli, ['--db', temp_db_path, 'list', 'show', 'hier-list', '--tree'])
...
def test_dependency_management(temp_db_path):
    ...
    runner.invoke(cli, ['--db', temp_db_path, 'item', 'add', 'backend', 'api', 'API endpoint'])
    runner.invoke(cli, ['--db', temp_db_path, 'item', 'add', 'frontend', 'ui', 'UI component'])

    # Add a dependency
    result = runner.invoke(cli, ['--db', temp_db_path, 'dep', 'add', 'frontend:ui', 'requires', 'backend:api'], input='y\n')
    assert result.exit_code == 0
    ...
    result = runner.invoke(cli, ['--db', temp_db_path, 'dep', 'remove', 'frontend:ui', 'backend:api'], input='y\n')
    assert result.exit_code == 0
