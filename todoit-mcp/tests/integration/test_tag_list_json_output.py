...
class TestTagListJsonOutput:
    ...
    def test_tag_list_json_output_with_tags(self):
        ...
        with self.runner.isolated_filesystem():
            # Create some tags
            result = self.runner.invoke(cli, ['--db', 'test.db', 'tag', 'create', 'work', '--color', 'blue'])
            assert result.exit_code == 0
            
            result = self.runner.invoke(cli, ['--db', 'test.db', 'tag', 'create', 'personal', '--color', 'green'])
            assert result.exit_code == 0
            
            result = self.runner.invoke(cli, ['--db', 'test.db', 'tag', 'create', 'urgent', '--color', 'red'])
            assert result.exit_code == 0
    ...
    def test_tag_list_json_output_specific_colors(self):
        ...
        with self.runner.isolated_filesystem():
            ...
            blue_tag = next(tag for tag in output_data['data'] if tag['Name'] == 'blue_tag')
            red_tag = next(tag for tag in output_data['data'] if tag['Name'] == 'red_tag')
    ...
    def test_tag_list_multiple_tags_alphabetical_order(self):
        ...
        with self.runner.isolated_filesystem():
            ...
            result = self.runner.invoke(cli, ['--db', 'test.db', 'tag', 'create', 'zebra'])
            assert result.exit_code == 0
            
            result = self.runner.invoke(cli, ['--db', 'test.db', 'tag', 'create', 'alpha'])
            assert result.exit_code == 0
            
            result = self.runner.invoke(cli, ['--db', 'test.db', 'tag', 'create', 'beta'])
            assert result.exit_code == 0
