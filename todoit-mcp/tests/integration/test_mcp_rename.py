...
from interfaces.mcp_server import todo_add_item, todo_rename_item, todo_rename_list
...
class TestMCPRename:
    ...
    async def test_mcp_rename_item_key_only(self, temp_db_path, setup_test_data):
        ...
        result = await todo_rename_item(
            list_key="mcp_project", item_key="task1", new_key="mcp_renamed_task"
        )
        ...
        assert (
            result["item"]["title"] == "Original Task 1"
        )  # MCP should return title, not content
    ...
    async def test_mcp_rename_item_title_only(self, temp_db_path, setup_test_data):
        ...
        result = await todo_rename_item(
            list_key="mcp_project", item_key="task1", new_title="MCP Updated Title"
        )
    ...
    @pytest.mark.asyncio
    async def test_mcp_rename_item_both_key_and_title(
        self, temp_db_path, setup_test_data
    ):
    ...
    @pytest.mark.asyncio
    async def test_mcp_rename_item_validation_errors(
        self, temp_db_path, setup_test_data
    ):
        ...
        result = await todo_rename_item(list_key="mcp_project", item_key="task1")
        ...
        # Test item not found
        result = await todo_rename_item(
            list_key="mcp_project", item_key="nonexistent", new_key="new_key"
        )
    ...
    async def test_mcp_rename_list_key_only(self, temp_db_path, setup_test_data):
        ...
        result = await todo_rename_list(list_key="mcp_project", new_key="mcp_key_only")
    ...
    async def test_mcp_rename_list_title_only(self, temp_db_path, setup_test_data):
        ...
        result = await todo_rename_list(
            list_key="mcp_project", new_title="MCP Title Only"
        )
    ...
    @pytest.mark.asyncio
    async def test_mcp_add_item_uses_title_parameter(
        self, temp_db_path, setup_test_data
    ):
        ...
        result = await todo_add_item(
            list_key="mcp_project", item_key="new_item", title="New Item Title"
        )
    ...
    @pytest.mark.asyncio
    async def test_mcp_add_subitem_uses_title_parameter(
        self, temp_db_path, setup_test_data
    ):
    ...
    @pytest.mark.asyncio
    async def test_mcp_content_to_title_mapping_consistency(
        self, temp_db_path, setup_test_data
    ):
        ...
        add_result = await todo_add_item(
            list_key="mcp_project", item_key="mapping_test", title="Mapping Test Item"
        )
    ...
    @pytest.mark.asyncio
    async def test_mcp_rename_subitem_duplicate_keys_different_parents(
        self, temp_db_path, setup_test_data
    ):
        ...
        manager.add_subitem(
            "mcp_project", "task3", "implementation", "Implementation Phase"
        )
        ...
        assert (
            result_subitem1_title["item"]["title"] == "Updated Design Phase for Task1"
        )
        ...
        assert (
            design_task1_updated.content == "Updated Design Phase for Task1"
        )  # Changed
        ...
        assert (
            result_subitem3_title["item"]["title"] == "Updated Design Phase for Task3"
        )
        ...
        assert (
            design_task3_updated.content == "Updated Design Phase for Task3"
        )  # Changed
        ...
        assert (
            design_task1_final.content == "Updated Design Phase for Task1"
        )  # From previous test
