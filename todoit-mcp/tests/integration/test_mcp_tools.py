...
class TestMCPToolsBasic:
    ...
    def test_mcp_tools_count(self):
        ...
        assert tool_count == 47, f"Expected exactly 47 MCP tools, found {tool_count}"
    ...
    def test_mcp_tool_names(self):
        ...
        result = subprocess.run(['grep', '-oE', 'async def todo_[a-z_]+', mcp_server_path], 
                              capture_output=True, text=True)
...
class TestMCPToolsFunctional:
    ...
    def test_item_operations(self, temp_manager):
        ...
        # Update content
        updated_content = temp_manager.update_item_content("item_test", "task1", "Updated task")
    ...
    def test_subtask_operations(self, temp_manager):
        ...
        if hasattr(temp_manager, 'add_subtask'):
            subtask = temp_manager.add_subtask("subtask_test", "parent", "sub1", "Subtask 1")
    ...
    def test_properties_operations(self, temp_manager):
        ...
        item_prop = temp_manager.set_item_property("prop_test", "item1", "size", "large")
    ...
    def test_quick_add(self, temp_manager):
        ...
        if hasattr(temp_manager, 'quick_add'):
            ...
            if isinstance(added[0], dict):
                ...
                assert "item_0001" in added[0]["item_key"] or added[0]["item_key"].startswith("item_")
            else:
                assert added[0].content == "Task A"
