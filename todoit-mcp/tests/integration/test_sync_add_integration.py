...
class TestSyncAddIntegration:
    ...
    def test_basic_sync_add(self, manager):
        ...
        parent_task3 = next((item for item in parent_items_after if item.item_key == "task3"), None)
        child_task3 = next((item for item in child_items_after if item.item_key == "task3"), None)
    ...
    def test_sync_add_with_properties(self, manager):
        ...
        manager.add_item("dev", "complex_task", "Complex task with properties", 
        ...
        test_sync = next((item for item in test_items if item.item_key == "sync_test"), None)
    ...
    def test_sync_add_multiple_children(self, manager):
        ...
        assert len(main_items_after) == len(child1_items_after) == len(child2_items_after) == 2
        
        # Verify the task exists in all lists
        main_multi = next((item for item in main_items_after if item.item_key == "multi_sync"), None)
        child1_multi = next((item for item in child1_items_after if item.item_key == "multi_sync"), None)
        child2_multi = next((item for item in child2_items_after if item.item_key == "multi_sync"), None)
    ...
    def test_no_sync_to_non_1to1_lists(self, manager):
        ...
        manager.create_list_relation(
            source_list.id, related_list.id, "project", "test_project",
    ...
    def test_sync_skip_existing_items(self, manager):
        ...
        child_items = manager.get_list_items("child")
        duplicate_in_child = next((item for item in child_items if item.item_key == "duplicate"), None)
    ...
    def test_sync_with_positions(self, manager):
        ...
        middle_in_copy = next((item for item in copy_items if item.item_key == "middle"), None)
