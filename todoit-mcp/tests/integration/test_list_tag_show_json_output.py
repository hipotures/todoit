...
class TestListTagShowJsonOutput:
    ...
    def test_list_tag_show_json_output_with_tags(self):
        ...
        with self.runner.isolated_filesystem():
            ...
            # Create some tags
            result = self.runner.invoke(cli, ['--db', 'test.db', 'tag', 'create', 'work'])
            assert result.exit_code == 0
            
            result = self.runner.invoke(cli, ['--db', 'test.db', 'tag', 'create', 'urgent'])
            assert result.exit_code == 0
            
            result = self.runner.invoke(cli, ['--db', 'test.db', 'tag', 'create', 'personal'])
            assert result.exit_code == 0
            
            # Assign tags to the list
            result = self.runner.invoke(cli, ['--db', 'test.db', 'list', 'tag', 'add', 'testlist', 'work'])
            assert result.exit_code == 0
            
            result = self.runner.invoke(cli, ['--db', 'test.db', 'list', 'tag', 'add', 'testlist', 'urgent'])
            assert result.exit_code == 0
            
            # Test JSON output for list tag show
            result = self.runner.invoke(cli, ['--db', 'test.db', 'list', 'tag', 'show', 'testlist'])
            assert result.exit_code == 0
    ...
    def test_list_tag_show_json_output_single_tag(self):
        ...
        with self.runner.isolated_filesystem():
            ...
            result = self.runner.invoke(cli, ['--db', 'test.db', 'tag', 'create', 'solo'])
            assert result.exit_code == 0
            
            result = self.runner.invoke(cli, ['--db', 'test.db', 'list', 'tag', 'add', 'testlist', 'solo'])
            assert result.exit_code == 0
    ...
    def test_list_tag_show_json_output_no_tags(self):
        ...
        with self.runner.isolated_filesystem():
            ...
            result = self.runner.invoke(cli, ['--db', 'test.db', 'list', 'create', 'emptylist', '--title', 'Empty List'])
            assert result.exit_code == 0
            
            # Test JSON output when no tags assigned
            result = self.runner.invoke(cli, ['--db', 'test.db', 'list', 'tag', 'show', 'emptylist'])
            assert result.exit_code == 0
    ...
    def test_list_tag_show_json_output_list_title_in_output(self):
        ...
        with self.runner.isolated_filesystem():
            ...
            result = self.runner.invoke(cli, ['--db', 'test.db', 'list', 'create', 'mylist', '--title', 'My Custom List'])
            assert result.exit_code == 0
            
            # Create and assign a tag
            result = self.runner.invoke(cli, ['--db', 'test.db', 'tag', 'create', 'testtag'])
            assert result.exit_code == 0
            
            result = self.runner.invoke(cli, ['--db', 'test.db', 'list', 'tag', 'add', 'mylist', 'testtag'])
            assert result.exit_code == 0
            
            # Test JSON output
            result = self.runner.invoke(cli, ['--db', 'test.db', 'list', 'tag', 'show', 'mylist'])
            assert result.exit_code == 0
    ...
    def test_list_tag_show_json_output_nonexistent_list(self):
        ...
        with self.runner.isolated_filesystem():
            ...
            result = self.runner.invoke(cli, ['--db', 'test.db', 'list', 'tag', 'show', 'nonexistent'])
    ...
    def test_list_tag_show_table_format_still_works(self):
        ...
        with self.runner.isolated_filesystem():
            ...
            result = self.runner.invoke(cli, ['--db', 'test.db', 'tag', 'create', 'test'])
            assert result.exit_code == 0
            
            result = self.runner.invoke(cli, ['--db', 'test.db', 'list', 'tag', 'add', 'testlist', 'test'])
            assert result.exit_code == 0
    ...
    def test_list_tag_show_yaml_format(self):
        ...
        with self.runner.isolated_filesystem():
            ...
            result = self.runner.invoke(cli, ['--db', 'test.db', 'tag', 'create', 'yaml_test'])
            assert result.exit_code == 0
            
            result = self.runner.invoke(cli, ['--db', 'test.db', 'list', 'tag', 'add', 'testlist', 'yaml_test'])
            assert result.exit_code == 0
    ...
    def test_list_tag_show_xml_format(self):
        ...
        with self.runner.isolated_filesystem():
            ...
            result = self.runner.invoke(cli, ['--db', 'test.db', 'tag', 'create', 'xml_test'])
            assert result.exit_code == 0
            
            result = self.runner.invoke(cli, ['--db', 'test.db', 'list', 'tag', 'add', 'testlist', 'xml_test'])
            assert result.exit_code == 0
    ...
    def test_list_tag_show_with_multiple_colors(self):
        ...
        with self.runner.isolated_filesystem():
            # Create a test list
            result = self.runner.invoke(cli, ['--db', 'test.db', 'list', 'create', 'colorlist', '--title', 'Color List'])
            assert result.exit_code == 0
            
            # Create tags with different colors
            result = self.runner.invoke(cli, ['--db', 'test.db', 'tag', 'create', 'red_tag', '--color', 'red'])
            assert result.exit_code == 0
            
            result = self.runner.invoke(cli, ['--db', 'test.db', 'tag', 'create', 'blue_tag', '--color', 'blue'])
            assert result.exit_code == 0
            
            result = self.runner.invoke(cli, ['--db', 'test.db', 'tag', 'create', 'green_tag', '--color', 'green'])
            assert result.exit_code == 0
            
            # Assign all tags to the list
            for tag in ['red_tag', 'blue_tag', 'green_tag']:
                result = self.runner.invoke(cli, ['--db', 'test.db', 'list', 'tag', 'add', 'colorlist', tag])
                assert result.exit_code == 0
            
            # Test JSON output
            result = self.runner.invoke(cli, ['--db', 'test.db', 'list', 'tag', 'show', 'colorlist'])
            assert result.exit_code == 0
            ...
            assert all(len(color) > 2 for color in colors)  # Should have "● " + color name