...
class TestItemTreeJsonOutput:
    ...
    def test_item_tree_json_output_entire_list(self):
        ...
        with self.runner.isolated_filesystem():
            ...
            result = self.runner.invoke(cli, ['--db', 'test.db', 'item', 'status', 'testlist', 'task2', '--status', 'in_progress'])
            assert result.exit_code == 0
            
            # Test JSON output for entire list tree
            result = self.runner.invoke(cli, ['--db', 'test.db', 'item', 'tree', 'testlist'])
            assert result.exit_code == 0
    ...
    def test_item_tree_json_output_specific_item(self):
        ...
        with self.runner.isolated_filesystem():
            ...
            # Add subtasks
            result = self.runner.invoke(cli, ['--db', 'test.db', 'item', 'add-subtask', 'testlist', 'parent', 'sub1', 'Subtask 1'])
            assert result.exit_code == 0
            
            result = self.runner.invoke(cli, ['--db', 'test.db', 'item', 'add-subtask', 'testlist', 'parent', 'sub2', 'Subtask 2'])
            assert result.exit_code == 0
            
            # Set some statuses
            result = self.runner.invoke(cli, ['--db', 'test.db', 'item', 'status', 'testlist', 'sub1', '--status', 'completed'])
            assert result.exit_code == 0
            
            # Test JSON output for specific item hierarchy
            result = self.runner.invoke(cli, ['--db', 'test.db', 'item', 'tree', 'testlist', 'parent'])
            assert result.exit_code == 0
            ...
            parent_items = [item for item in output_data['data'] if item['Level'] == '0']
            ...
            subtask_items = [item for item in output_data['data'] if item['Level'] == '1']
    ...
    def test_item_tree_json_output_empty_list(self):
        ...
        with self.runner.isolated_filesystem():
            ...
            result = self.runner.invoke(cli, ['--db', 'test.db', 'item', 'tree', 'emptylist'])
            assert result.exit_code == 0
    ...
    def test_item_tree_json_output_hierarchy_display(self):
        ...
        with self.runner.isolated_filesystem():
            ...
            # Add a subtask
            result = self.runner.invoke(cli, ['--db', 'test.db', 'item', 'add-subtask', 'testlist', 'parent', 'child', 'Child Task'])
            assert result.exit_code == 0
            ...
            parent_item = next(item for item in output_data['data'] if item['Level'] == '0')
            assert '📋' in parent_item['Hierarchy']
            assert 'parent:' in parent_item['Hierarchy']
            
            # Check child item hierarchy display
            child_item = next(item for item in output_data['data'] if item['Level'] == '1')
    ...
    def test_item_tree_json_output_nonexistent_item(self):
        ...
        with self.runner.isolated_filesystem():
            ...
            result = self.runner.invoke(cli, ['--db', 'test.db', 'item', 'tree', 'testlist', 'nonexistent'])
    ...
    def test_item_tree_positions_and_keys_present(self):
        ...
        with self.runner.isolated_filesystem():
            ...
            result = self.runner.invoke(cli, ['--db', 'test.db', 'item', 'add', 'testlist', 'first', 'First Task'])
            assert result.exit_code == 0
            
            result = self.runner.invoke(cli, ['--db', 'test.db', 'item', 'add', 'testlist', 'second', 'Second Task'])
            assert result.exit_code == 0
