...
class TestCLICreateListWithTags:
    ...
    def test_cli_create_list_with_multiple_tags(self, runner, temp_db, manager_with_tags):
    ...
    def test_cli_create_list_with_nonexistent_tag(self, runner, temp_db, manager_with_tags):
        ...
        assert result.exit_code == 0  # CLI doesn't exit with error code, just shows message
    ...
    def test_cli_create_list_with_mixed_valid_invalid_tags(self, runner, temp_db, manager_with_tags):
    ...
    def test_cli_create_list_with_tags_and_items(self, runner, temp_db, manager_with_tags):
        """Test CLI create list with both tags and items"""
        result = runner.invoke(cli, [
            '--db-path', temp_db,
            'list', 'create',
            '--list', 'project-with-tasks',
    ...
    def test_cli_create_list_with_tags_and_metadata(self, runner, temp_db, manager_with_tags):
    ...
    def test_cli_create_list_case_insensitive_tags(self, runner, temp_db, manager_with_tags):
        ...
        result = runner.invoke(cli, [
            ...
            '--list', 'case-test',
            '--title', 'Case Test',
            '--tag', 'FRONTEND',  # uppercase
            '--tag', 'Backend'    # mixed case
        ])
    ...
    def test_cli_create_list_with_from_folder_and_tags(self, runner, temp_db, manager_with_tags):
    ...
    def test_cli_create_list_duplicate_tags_in_command(self, runner, temp_db, manager_with_tags):
        ...
        result = runner.invoke(cli, [
            ...
            '--tag', 'frontend',
            '--tag', 'frontend',  # duplicate
            '--tag', 'FRONTEND'   # case variant
        ])
        
        assert result.exit_code == 0
        assert 'List Created' in result.output
        assert 'Tags: frontend, frontend, FRONTEND' in result.output  # Shows input as-is
