...
class TestSubtaskPositioningFix:
    ...
    def test_subtask_positioning_no_conflicts(self, runner):
        ...
        with runner.isolated_filesystem():
            # Create list
            result = self.run_cli_old(runner, ['list', 'create', '--list', 'test_positioning', '--title', 'Test Positioning'])
            assert result.exit_code == 0

            # Add multiple main tasks
            for i in range(1, 4):
                result = self.run_cli_old(runner, ['item', 'add', '--list', 'test_positioning', '--item', f'item{i}', '--title', f'Item {i}'])
                assert result.exit_code == 0

            # Add multiple subtasks to each task
            for task_num in range(1, 4):
                for sub_num in range(1, 4):
                    result = self.run_cli_old(runner, ['item', 'add', '--list', 'test_positioning', '--item', f'item{task_num}', '--subitem', f'item{task_num}_sub{sub_num}', '--title', f'Subitem {sub_num} for Item {task_num}'])
                    assert result.exit_code == 0, f"Failed to add subitem {sub_num} to item {task_num}: {result.output}"

            # Verify all items are displayed correctly
            result = self.run_cli_old(runner, ['list', 'show', '--list', 'test_positioning'])
            assert result.exit_code == 0
    ...
    def test_database_positions_are_sequential(self, temp_db):
        ...
        self.run_cli('item add --list test_db --item task1 --subitem sub1 --title "Sub 1"', temp_db)
        self.run_cli('item add --list test_db --item task2 --subitem sub2 --title "Sub 2"', temp_db)
        self.run_cli('item add --list test_db --item task1 --subitem sub3 --title "Sub 3"', temp_db)
    ...
    def test_mixed_task_and_subtask_creation(self, temp_db):
        ...
        # Create task1
        result = self.run_cli('item add --list test_mixed --item task1 --title "Item 1"', temp_db)
        assert result.exit_code == 0
        
        # Add subitem to task1
        result = self.run_cli('item add --list test_mixed --item task1 --subitem sub1 --title "Sub 1"', temp_db)
        assert result.exit_code == 0
        
        # Create task2 (after subitem)
        result = self.run_cli('item add --list test_mixed --item task2 --title "Item 2"', temp_db)
        assert result.exit_code == 0
        
        # Add subitem to task2
        result = self.run_cli('item add --list test_mixed --item task2 --subitem sub2 --title "Sub 2"', temp_db)
        assert result.exit_code == 0
        
        # Add another subitem to task1
        result = self.run_cli('item add --list test_mixed --item task1 --subitem sub3 --title "Sub 3"', temp_db)
        assert result.exit_code == 0
    ...
    def test_hierarchy_organization_with_fixed_positioning(self, temp_db):
        ...
        self.run_cli('list create --list test_hierarchy --title "Test Hierarchy"', temp_db)
        self.run_cli('item add --list test_hierarchy --item main1 --title "Main 1"', temp_db)
        self.run_cli('item add --list test_hierarchy --item main2 --title "Main 2"', temp_db)
        self.run_cli('item add --list test_hierarchy --item main1 --subitem sub1_1 --title "Sub 1.1"', temp_db)
        self.run_cli('item add --list test_hierarchy --item main1 --subitem sub1_2 --title "Sub 1.2"', temp_db)
        self.run_cli('item add --list test_hierarchy --item main2 --subitem sub2_1 --title "Sub 2.1"', temp_db)
