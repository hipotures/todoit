...
class TestMCPTagIntegration:
    ...
    async def test_mcp_tag_creation_flow(self, integration_manager):
        ...
        result = await todo_add_list_tag(
            "test_project", "work", mgr=integration_manager
        )
        assert result["success"] is True
    ...
    async def test_mcp_tag_removal(self, integration_manager):
        ...
        # Remove tag via MCP
        result = await todo_remove_list_tag(
            "test_list", "temp_tag", mgr=integration_manager
        )
        assert result["success"] is True
...
class TestCLITagIntegration:
    ...
    def test_cli_environment_variable_integration(
        self, integration_manager, monkeypatch
    ):
...
class TestTagFilteringEdgeCases:
    ...
    def test_archived_lists_with_tags(self, integration_manager):
        ...
        all_work_lists = integration_manager.list_all(
            filter_tags=["work"], include_archived=True
        )
