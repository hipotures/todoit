...
def setup_duplicate_subitems(temp_db_path):
    ...
    manager.add_subitem(
        "test_list", "scene_0001", "image_gen", "Image generation for scene 1"
    )
    manager.add_subitem(
        "test_list", "scene_0001", "audio_sync", "Audio sync for scene 1"
    )

    manager.add_subitem(
        "test_list", "scene_0002", "image_gen", "Image generation for scene 2"
    )
    manager.add_subitem(
        "test_list", "scene_0002", "audio_sync", "Audio sync for scene 2"
    )
...
class TestCLISubitemDisambiguation:
            ...
            with patch(
                ...
                result = runner.invoke(
                    ...
                    [
                        ...
                        "audio_sync",
                        "--title",
                        "UPDATED: Audio sync for scene 1",
                    ],
                    obj={"db_path": db_path},
                )
    ...
    def test_item_delete_targets_correct_subitem(self, setup_duplicate_subitems):
        ...
        with patch(
            ...
            with patch(
                ...
                assert result.exit_code == 0
                assert (
                    "Subitem 'image_gen' deleted from list 'test_list'" in result.output
                )
    ...
    def test_item_state_clear_targets_correct_subitem(self, setup_duplicate_subitems):
        ...
        manager.update_item_status(
            ...
            subitem_key="image_gen",
            status="in_progress",
            completion_states={"reviewed": True},
        )
        manager.update_item_status(
            "test_list",
            "scene_0002",
        ...
        with patch(
            ...
            with patch(
                ...
                assert result.exit_code == 0
                assert (
                    "Cleared all completion states from subitem 'image_gen'"
                    in result.output
                )
...
class TestManagerFunctionsWithParentKey:
    ...
    def test_delete_item_with_parent_key(self, setup_duplicate_subitems):
        ...
        success = manager.delete_item(
            "test_list", "audio_sync", parent_item_key="scene_0001"
        )
    ...
    def test_clear_item_completion_states_with_parent_key(
        self, setup_duplicate_subitems
    ):
        ...
        manager.update_item_status(
            ...
            subitem_key="image_gen",
            status="in_progress",
            completion_states={"state1": True, "state2": False},
        )
        manager.update_item_status(
            "test_list",
            "scene_0002",
