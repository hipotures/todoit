...
class TestSubtaskPositioning:
    ...
    def test_add_subitem_uses_next_position(
        self, mock_manager, mock_list, mock_parent_item, mock_db_item
    ):
        ...
        mock_manager.db.get_item_by_key_and_parent.return_value = (
            None  # subitem doesn't exist for this parent
        )
        ...
        mock_manager.db.get_children_status_summary.return_value = (
            None  # No children yet
        )

        # Call add_subitem
        result = mock_manager.add_subitem(
            "test_list", "parent_key", "sub_key", "Sub content"
        )
    ...
    def test_add_subitem_no_position_shifting(
        self, mock_manager, mock_list, mock_parent_item, mock_db_item
    ):
    ...
    def test_add_subitem_ignores_existing_subtasks_positions(
        self, mock_manager, mock_list, mock_parent_item, mock_db_item
    ):
        ...
        existing_subtasks = [Mock(position=3), Mock(position=7), Mock(position=9)]

        mock_manager.db.get_list_by_key.return_value = mock_list
        mock_manager.db.get_item_by_key.return_value = mock_parent_item
        mock_manager.db.get_item_by_key_and_parent.return_value = None
        mock_manager.db.get_item_children.return_value = (
            existing_subtasks  # This should be ignored now
        )
    ...
    def test_add_subitem_sequential_calls_increment_position(
        self, mock_manager, mock_list, mock_parent_item, mock_db_item
    ):
    ...
    def test_add_subitem_metadata_preserved(
        self, mock_manager, mock_list, mock_parent_item, mock_db_item
    ):
        ...
        mock_manager.add_subitem(
            "test_list", "parent_key", "sub_key", "Sub content", test_metadata
        )
