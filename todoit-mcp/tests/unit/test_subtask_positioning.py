"""
Unit tests for subtask positioning logic
Tests the fix for position conflicts in add_subtask method
"""
...
class TestSubtaskPositioning:
    """Test subtask positioning functionality"""
    ...
    def test_add_subtask_uses_next_position(self, mock_manager, mock_list, mock_parent_item, mock_db_item):
        """Test that add_subtask always uses get_next_position() to avoid conflicts"""
        ...
        # Call add_subtask
        result = mock_manager.add_subtask("test_list", "parent_key", "sub_key", "Sub content")

        # Verify get_next_position was called (not positional logic)
        mock_manager.db.get_next_position.assert_called_once_with(1)
    ...
    def test_add_subtask_no_position_shifting(self, mock_manager, mock_list, mock_parent_item, mock_db_item):
        """Test that add_subtask no longer shifts positions (old buggy behavior)"""
        ...
        # Call add_subtask
        mock_manager.add_subtask("test_list", "parent_key", "sub_key", "Sub content")

        # Verify shift_positions was NOT called (old behavior would call this)
        mock_manager.db.shift_positions.assert_not_called()

    def test_add_subtask_ignores_existing_subtasks_positions(self, mock_manager, mock_list, mock_parent_item, mock_db_item):
        """Test that add_subtask ignores existing subtasks' positions (old logic would check these)"""
    ...
    def test_add_subtask_sequential_calls_increment_position(self, mock_manager, mock_list, mock_parent_item, mock_db_item):
        """Test that sequential add_subtask calls get incrementing positions"""
        ...
        # Call add_subtask twice
        mock_manager.add_subtask("test_list", "parent_key", "sub1", "Sub 1")
        mock_manager.add_subtask("test_list", "parent_key", "sub2", "Sub 2")
    ...
    def test_add_subtask_validates_inputs_before_positioning(self, mock_manager):
        ...
        with pytest.raises(ValueError, match="List 'bad_list' does not exist"):
            mock_manager.add_subtask("bad_list", "parent_key", "sub_key", "Sub content")
    ...
    def test_add_subtask_metadata_preserved(self, mock_manager, mock_list, mock_parent_item, mock_db_item):
        ...
        # Call add_subtask with metadata
        test_metadata = {"priority": "high", "tags": ["urgent"]}
        mock_manager.add_subtask("test_list", "parent_key", "sub_key", "Sub content", test_metadata)
