...
def manager_with_duplicate_subitems(temp_db_path):
    ...
    manager.add_item(
        "0014_jane_eyre_subtask", "scene_0001", "Generate image using scene_01.yaml"
    )
    manager.add_item(
        "0014_jane_eyre_subtask", "scene_0002", "Generate image using scene_02.yaml"
    )

    # Add subitems with same names - this is where the bug occurred
    manager.add_subitem(
        "0014_jane_eyre_subtask",
        "scene_0001",
        "scene_gen",
    ...
    manager.add_subitem(
        ...
        "scene_0001",
        "scene_style",
        "Scene styling for scene_0001.yaml",
    )
    manager.add_subitem(
        "0014_jane_eyre_subtask",
        "scene_0001",
        "image_gen",
        "Image generation for scene_0001.yaml",
    )
    ...
    manager.add_subitem(
        ...
        "scene_0002",
        "scene_gen",
        "Scene generation for scene_0002.yaml",
    )
    manager.add_subitem(
        "0014_jane_eyre_subtask",
        "scene_0002",
        "scene_style",
        "Scene styling for scene_0002.yaml",
    )
    manager.add_subitem(
        "0014_jane_eyre_subtask",
        "scene_0002",
        "image_gen",
        "Image generation for scene_0002.yaml",
    )
    manager.add_subitem(
        "0014_jane_eyre_subtask",
        "scene_0002",
        "image_dwn",
        "Image download for scene_0002.yaml",
    )
...
class TestOriginalBugScenario:
    ...
    def test_update_status_targets_correct_subitem(
        self, manager_with_duplicate_subitems
    ):
        """
        ...
        scene1_image_gen = manager.get_item(
            "0014_jane_eyre_subtask", "image_gen", "scene_0001"
        )
        scene2_image_gen = manager.get_item(
            "0014_jane_eyre_subtask", "image_gen", "scene_0002"
        )
        ...
        scene1_image_gen_after = manager.get_item(
            "0014_jane_eyre_subtask", "image_gen", "scene_0001"
        )
        scene2_image_gen_after = manager.get_item(
            "0014_jane_eyre_subtask", "image_gen", "scene_0002"
        )
    ...
    def test_edit_content_targets_correct_subitem(
        self, manager_with_duplicate_subitems
    ):
        ...
        # Verify correct subitem was updated
        scene1_scene_gen = manager.get_item(
            "0014_jane_eyre_subtask", "scene_gen", "scene_0001"
        )
        scene2_scene_gen = manager.get_item(
            "0014_jane_eyre_subtask", "scene_gen", "scene_0002"
        )

        assert (
            scene1_scene_gen.content == "Scene generation for scene_0001.yaml"
        )  # Unchanged
        assert (
            scene2_scene_gen.content == "UPDATED: Scene generation for scene_0002.yaml"
        )  # Updated
    ...
    def test_delete_targets_correct_subitem(self, manager_with_duplicate_subitems):
        ...
        scene1_scene_style = manager.get_item(
            "0014_jane_eyre_subtask", "scene_style", "scene_0001"
        )
        scene2_scene_style = manager.get_item(
            "0014_jane_eyre_subtask", "scene_style", "scene_0002"
        )
...
class TestMultipleDuplicatesAcrossParents:
    ...
    def test_complex_hierarchy_with_duplicates(self, temp_db_path):
        ...
        for parent in ["frontend", "backend", "mobile"]:
            manager.add_subitem(
                "complex_test", parent, "design", f"Design phase for {parent}"
            )
            manager.add_subitem(
                "complex_test", parent, "implement", f"Implementation for {parent}"
            )
            manager.add_subitem("complex_test", parent, "test", f"Testing for {parent}")
            manager.add_subitem(
                "complex_test", parent, "deploy", f"Deployment for {parent}"
            )

        # Complete design phase for backend only
        manager.update_item_status(
            "complex_test", "backend", subitem_key="design", status="completed"
        )

        # Start implementation for frontend only
        manager.update_item_status(
            "complex_test", "frontend", subitem_key="implement", status="in_progress"
        )

        # Fail testing for mobile only
        manager.update_item_status(
            "complex_test", "mobile", subitem_key="test", status="failed"
        )
...
class TestErrorConditionsWithDuplicates:
    ...
    def test_nonexistent_parent_with_duplicate_subitems(
        self, manager_with_duplicate_subitems
    ):
    ...
    def test_nonexistent_subitem_under_existing_parent(
        self, manager_with_duplicate_subitems
    ):
        ...
        scene2_image_dwn = manager.get_item(
            "0014_jane_eyre_subtask", "image_dwn", "scene_0002"
        )
    ...
    def test_completion_states_with_duplicates(self, manager_with_duplicate_subitems):
        ...
        scene1_item = manager.get_item(
            "0014_jane_eyre_subtask", "image_gen", "scene_0001"
        )
        scene2_item = manager.get_item(
            "0014_jane_eyre_subtask", "image_gen", "scene_0002"
        )
        ...
        scene1_after = manager.get_item(
            "0014_jane_eyre_subtask", "image_gen", "scene_0001"
        )
        scene2_after = manager.get_item(
            "0014_jane_eyre_subtask", "image_gen", "scene_0002"
        )

        assert not scene1_after.completion_states  # Cleared
        assert scene2_after.completion_states == {
            "tested": True,
            "deployed": False,
        }  # Unchanged
