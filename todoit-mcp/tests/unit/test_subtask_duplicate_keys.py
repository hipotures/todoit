...
def test_duplicate_subtask_keys_different_parents(temp_db_path):
    ...
    manager.add_subitem(
        "test_list", "scene_0020", "image_gen", "Image generation"
    )  # Same key as above
    manager.add_subitem(
        "test_list", "scene_0020", "image_dwn", "Image download"
    )  # Same key as above
...
def test_duplicate_subtask_keys_same_parent_fails(temp_db_path):
    ...
    with pytest.raises(
        ValueError,
        match="Subitem key 'image_gen' already exists for parent 'scene_0019'",
    ):
        manager.add_subitem(
            "test_list", "scene_0019", "image_gen", "Another image generation"
        )
...
def test_main_task_keys_still_unique(temp_db_path):
    ...
    with pytest.raises(
        ValueError, match="Item 'scene_0019' already exists in list 'test_list'"
    ):
