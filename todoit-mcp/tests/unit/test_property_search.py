...
class TestPropertySearchUnit:
    ...
    def test_find_items_by_property_success(self, manager_with_mock, mock_db):
        ...
        mock_item1 = MagicMock(
            ...
            position=1, created_at=datetime(2024, 1, 1)
        )
        mock_item2 = MagicMock(
            id=2, item_key="task2", content="Second task", status="pending", 
            position=2, created_at=datetime(2024, 1, 2)
        )
        ...
        mock_todo2 = TodoItem(
            id=2, list_id=1, item_key="task2", content="Second task", status=ItemStatus.PENDING,
            position=2, created_at=datetime(2024, 1, 2), updated_at=datetime(2024, 1, 2)
        )
        
        with patch.object(manager_with_mock, '_db_to_model', side_effect=[mock_todo1, mock_todo2]):
            result = manager_with_mock.find_items_by_property("test", "priority", "high")
        ...
        mock_db.find_items_by_property.assert_called_once_with(1, "priority", "high", None)
    ...
    def test_find_items_by_property_with_limit(self, manager_with_mock, mock_db):
        ...
        with patch.object(manager_with_mock, '_db_to_model', return_value=mock_todo):
            result = manager_with_mock.find_items_by_property("test", "status", "reviewed", limit=1)
        
        assert len(result) == 1
        mock_db.find_items_by_property.assert_called_once_with(1, "status", "reviewed", 1)
    ...
    def test_find_items_by_property_empty_result(self, manager_with_mock, mock_db):
        ...
        result = manager_with_mock.find_items_by_property("test", "nonexistent", "value")
