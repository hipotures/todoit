...
def test_demonstrates_original_bug_behavior():
    ...
    try:
        ...
        manager.add_subitem(
            "test_list", "scene_0001", "image_gen", "Image gen for scene 1"
        )
        manager.add_subitem(
            "test_list", "scene_0002", "image_gen", "Image gen for scene 2"
        )
        ...
        print(
            f"   scene_0001/image_gen status: {scene1_item.status} (should be 'pending')"
        )
        print(
            f"   scene_0002/image_gen status: {scene2_item.status} (should be 'failed')"
        )
...
def test_shows_old_vs_new_behavior_side_by_side():
    ...
    try:
        ...
        manager.update_item_status(
            "demo", "parent2", subitem_key="task", status="completed"
        )
...
def test_all_fixed_functions_work_correctly():
    ...
    try:
        ...
        for group in ["groupA", "groupB"]:
            manager.add_subitem(
                "all_funcs", group, "shared_task", f"Shared task in {group}"
            )
            manager.add_subitem(
                "all_funcs", group, "unique_task", f"Unique task in {group}"
            )
        ...
        manager.update_item_status(
            "all_funcs", "groupA", subitem_key="shared_task", status="in_progress"
        )
        ...
        manager.update_item_content(
            "all_funcs",
            "shared_task",
            "UPDATED: Shared task in groupB",
            parent_item_key="groupB",
        )
        ...
        manager.update_item_status(
            ...
            "groupA",
            status="in_progress",
            subitem_key="shared_task",
            completion_states={"test": True},
        )
        manager.update_item_status(
            "all_funcs",
            "groupB",
            status="in_progress",
            subitem_key="shared_task",
            completion_states={"review": True},
        )

        # Clear states from groupA only
        manager.clear_item_completion_states(
            "all_funcs", "shared_task", parent_item_key="groupA"
        )
        ...
        success = manager.delete_item(
            "all_funcs", "unique_task", parent_item_key="groupA"
        )
