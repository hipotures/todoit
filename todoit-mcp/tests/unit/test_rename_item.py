...
class TestRenameItemUnit:
    ...
    def test_rename_item_key_only(self, manager_with_mock, mock_db, sample_list_db, sample_item_db):
        ...
        assert mock_db.get_item_by_key_and_parent.call_count == 2  # First for item lookup, second for uniqueness check
    ...
    def test_rename_item_title_only(self, manager_with_mock, mock_db, sample_list_db, sample_item_db):
        ...
        # Setup
        mock_db.get_list_by_key.return_value = sample_list_db
        mock_db.get_item_by_key_and_parent.return_value = sample_item_db  # Updated to use unified API
        ...
        # Execute
        result = manager_with_mock.rename_item("project", "old_task", new_content="New Task Title")
    ...
    def test_rename_item_both_key_and_title(self, manager_with_mock, mock_db, sample_list_db, sample_item_db):
        ...
        # Setup
        mock_db.get_list_by_key.return_value = sample_list_db
        mock_db.get_item_by_key_and_parent.side_effect = [sample_item_db, None]  # First for item, second for uniqueness check
        mock_db.update_item.return_value = sample_item_db
        ...
        result = manager_with_mock.rename_item(
            "project", "old_task", 
            new_key="new_task", 
            new_content="New Task Title"
        )
        
        # Verify
        mock_db.update_item.assert_called_with(10, {"item_key": "new_task", "content": "New Task Title"})
        manager_with_mock._record_history.assert_called_once()
        assert result == mock_todo_item
    ...
    def test_rename_subitem(self, manager_with_mock, mock_db, sample_list_db, 
        ...
        mock_db.update_item.assert_called_with(30, {"item_key": "new_subtask", "content": "New Subtask Title"})
    ...
    def test_rename_subitem_not_found_error(self, manager_with_mock, mock_db, 
        ...
        with pytest.raises(ValueError, match="Item 'nonexistent' not found under parent 'parent_task' in list 'project'"):
    ...
    def test_rename_duplicate_key_error_item(self, manager_with_mock, mock_db, 
        ...
        mock_db.get_list_by_key.return_value = sample_list_db
        mock_db.get_item_by_key_and_parent.side_effect = [sample_item_db, existing_item]  # First call gets item, second checks for duplicate
        
        with pytest.raises(ValueError, match="Item with key 'existing_task' already exists in list 'project'"):
    ...
    def test_rename_duplicate_key_error_subitem(self, manager_with_mock, mock_db, 
        ...
        with pytest.raises(ValueError, match="Item with key 'existing_subtask' already exists under parent 'parent_task' in list 'project'"):
            manager_with_mock.rename_item(
                "project", "old_subtask", 
    ...
    def test_rename_same_key_allowed(self, manager_with_mock, mock_db, 
        ...
        # Setup
        mock_db.get_list_by_key.return_value = sample_list_db
        mock_db.get_item_by_key_and_parent.return_value = sample_item_db  # Same item for both calls
        mock_db.update_item.return_value = sample_item_db
    ...
    def test_rename_database_failure_error(self, manager_with_mock, mock_db, 
        ...
        mock_db.get_list_by_key.return_value = sample_list_db
        mock_db.get_item_by_key_and_parent.side_effect = [sample_item_db, None]  # First call finds item, second for uniqueness check returns None
        mock_db.update_item.return_value = None  # Simulate failure
        
        with pytest.raises(AttributeError):  # update_item failure would cause AttributeError when accessing None.item_key
    ...
    def test_rename_history_recording_item(self, manager_with_mock, mock_db, 
        ...
        # Execute
        manager_with_mock.rename_item("project", "old_task", new_key="new_task", new_content="New Title")
    ...
    def test_rename_key_validation_alphanumeric(self, manager_with_mock, mock_db, 
        ...
        # Setup
        mock_db.get_list_by_key.return_value = sample_list_db
        mock_db.get_item_by_key_and_parent.side_effect = [sample_item_db, None] * 5  # Return pattern for each valid key test
        mock_db.update_item.return_value = sample_item_db
