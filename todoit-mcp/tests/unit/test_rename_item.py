...
class TestRenameItemUnit:
    ...
    def test_rename_item_key_only(self, manager_with_mock, mock_db, sample_list_db, sample_item_db):
        ...
        mock_db.get_item_by_key.side_effect = [sample_item_db, None]
        mock_db.rename_item.return_value = sample_item_db
        ...
        # Execute
        result = manager_with_mock.rename_item("project", "old_task", new_key="new_task")
        
        # Verify
        mock_db.get_list_by_key.assert_called_with("project")
        assert mock_db.get_item_by_key.call_count == 2  # First for item lookup, second for uniqueness check
        mock_db.rename_item.assert_called_with(10, "new_task", None)
        manager_with_mock._record_history.assert_called_once()
        assert result == mock_todo_item

    def test_rename_item_title_only(self, manager_with_mock, mock_db, sample_list_db, sample_item_db):
        """Test renaming only the item title"""
        # Setup
        mock_db.get_list_by_key.return_value = sample_list_db
        mock_db.get_item_by_key.return_value = sample_item_db  # Only one call for item lookup
        mock_db.rename_item.return_value = sample_item_db
        ...
        # Execute
        result = manager_with_mock.rename_item("project", "old_task", new_title="New Task Title")
        
        # Verify
        mock_db.rename_item.assert_called_with(10, None, "New Task Title")
        manager_with_mock._record_history.assert_called_once()
        assert result == mock_todo_item

    def test_rename_item_both_key_and_title(self, manager_with_mock, mock_db, sample_list_db, sample_item_db):
        """Test renaming both key and title"""
        # Setup
        mock_db.get_list_by_key.return_value = sample_list_db
        mock_db.get_item_by_key.side_effect = [sample_item_db, None]  # First for item, second for uniqueness check
        mock_db.rename_item.return_value = sample_item_db
        ...
        # Verify
        mock_db.rename_item.assert_called_with(10, "new_task", "New Task Title")
        manager_with_mock._record_history.assert_called_once()
        assert result == mock_todo_item

    def test_rename_subitem(self, manager_with_mock, mock_db, sample_list_db, 
                           sample_parent_item_db, sample_subitem_db):
        ...
        mock_db.rename_item.return_value = sample_subitem_db
        
        # Execute
        result = manager_with_mock.rename_item(
            "project", "old_subtask", 
            new_key="new_subtask", 
            new_title="New Subtask Title",
            parent_item_key="parent_task"
        )
        
        # Verify
        mock_db.get_item_by_key.assert_called_with(1, "parent_task")
        mock_db.rename_item.assert_called_with(30, "new_subtask", "New Subtask Title")
    ...
    def test_rename_item_not_found_error(self, manager_with_mock, mock_db, sample_list_db):
        """Test error when item doesn't exist"""
        mock_db.get_list_by_key.return_value = sample_list_db
        mock_db.get_item_by_key.return_value = None
        
        with pytest.raises(ValueError, match="Item 'nonexistent' not found in list 'project'"):
            manager_with_mock.rename_item("project", "nonexistent", new_key="new_task")

    def test_rename_subitem_not_found_error(self, manager_with_mock, mock_db, 
                                          sample_list_db, sample_parent_item_db):
        ...
        with pytest.raises(ValueError, match="Subitem 'nonexistent' not found"):
    ...
    def test_rename_parent_not_found_error(self, manager_with_mock, mock_db, sample_list_db):
        """Test error when parent item doesn't exist for subitem"""
        mock_db.get_list_by_key.return_value = sample_list_db
        mock_db.get_item_by_key.return_value = None  # Parent not found
        
        with pytest.raises(ValueError, match="Parent item 'nonexistent_parent' not found"):
            manager_with_mock.rename_item(
                "project", "subtask1", 
                new_key="new_subtask",
    ...
    def test_rename_duplicate_key_error_item(self, manager_with_mock, mock_db, 
                                           sample_list_db, sample_item_db):
        ...
        mock_db.get_list_by_key.return_value = sample_list_db
        mock_db.get_item_by_key.return_value = sample_item_db
        mock_db.get_item_by_key_and_parent.return_value = existing_item  # Key exists
        
        with pytest.raises(ValueError, match="Item key 'existing_task' already exists"):
            manager_with_mock.rename_item("project", "old_task", new_key="existing_task")

    def test_rename_duplicate_key_error_subitem(self, manager_with_mock, mock_db, 
                                              sample_list_db, sample_parent_item_db, sample_subitem_db):
        ...
        with pytest.raises(ValueError, match="Subitem key 'existing_subtask' already exists"):
            manager_with_mock.rename_item(
                "project", "old_subtask", 
    ...
    def test_rename_same_key_allowed(self, manager_with_mock, mock_db, 
                                   sample_list_db, sample_item_db):
        """Test that renaming to the same key is allowed (for title-only changes)"""
        # Setup
        mock_db.get_list_by_key.return_value = sample_list_db
        mock_db.get_item_by_key.return_value = sample_item_db
        mock_db.get_item_by_key_and_parent.return_value = sample_item_db  # Same item
        mock_db.rename_item.return_value = sample_item_db
        ...
        mock_db.rename_item.assert_called_with(10, "old_task", "New Title")

    def test_rename_database_failure_error(self, manager_with_mock, mock_db, 
                                         sample_list_db, sample_item_db):
        """Test error when database rename fails"""
        mock_db.get_list_by_key.return_value = sample_list_db
        mock_db.get_item_by_key.side_effect = [sample_item_db, None]  # First call finds item, second for uniqueness check returns None
        mock_db.rename_item.return_value = None  # Simulate failure
        
        with pytest.raises(ValueError, match="Failed to rename item"):
            manager_with_mock.rename_item("project", "old_task", new_key="new_task")

    def test_rename_history_recording_item(self, manager_with_mock, mock_db, 
                                         sample_list_db, sample_item_db):
        ...
        mock_db.rename_item.return_value = renamed_item
        
        # Execute
        manager_with_mock.rename_item("project", "old_task", new_key="new_task", new_title="New Title")
        
        # Verify history call
        call_args = manager_with_mock._record_history.call_args
        assert call_args[1]['item_id'] == 10
        assert call_args[1]['action'] == "rename_item"
        assert call_args[1]['old_value'] == {"item_key": "old_task", "content": "Old Task Content"}
        assert call_args[1]['new_value']['item_key'] == "new_task"
        assert call_args[1]['new_value']['content'] == "New Title"

    def test_rename_history_recording_subitem(self, manager_with_mock, mock_db, 
                                            sample_list_db, sample_parent_item_db, sample_subitem_db):
        ...
        renamed_subitem.content = "Old Subtask Content"  # Content unchanged in this test
        mock_db.rename_item.return_value = renamed_subitem
    ...
    def test_rename_key_validation_alphanumeric(self, manager_with_mock, mock_db, 
                                              sample_list_db, sample_item_db):
        """Test that item key validation allows alphanumeric with underscores and hyphens"""
        # Setup
        mock_db.get_list_by_key.return_value = sample_list_db
        mock_db.get_item_by_key.side_effect = [sample_item_db, None] * 5  # Return pattern for each valid key test
        mock_db.rename_item.return_value = sample_item_db
        ...
        for valid_key in valid_keys:
            result = manager_with_mock.rename_item("project", "old_task", new_key=valid_key)
            assert result == mock_todo_item

    def test_rename_preserves_item_relationships(self, manager_with_mock, mock_db, 
                                               sample_list_db, sample_item_db):
        ...
        # Verify that rename_item was called with only the key/content changes
        mock_db.rename_item.assert_called_with(10, "new_task", None)
    ...
    def test_rename_validation_flow(self, manager_with_mock, mock_db, 
                                  sample_list_db, sample_item_db):
        ...
        call_order = [
            ...
            mock_db.get_item_by_key,  # 2. Validate item exists
            mock_db.rename_item,  # 3. Perform rename
        ]
