...
class TestTodoManagerLink:
    ...
    def test_link_list_basic(self, manager, sample_source_list):
        ...
        # Mock methods
        manager.get_list = Mock(
            side_effect=lambda key: sample_source_list if key == "source_list" else None
        )
        manager.create_list = Mock(
            return_value=TodoList(
                id=2,
                list_key="target_list",
                title="Source List - Linked",
                created_at=datetime.now(),
                updated_at=datetime.now(),
            )
        )
    ...
    def test_link_list_with_properties(self, manager, sample_source_list):
        ...
        manager.get_list_properties = Mock(
            return_value={"prop1": "value1", "prop2": "value2"}
        )
    ...
    def test_link_list_with_item_properties(
        self, manager, sample_source_list, sample_items
    ):
    ...
    def test_link_list_source_not_exists(self, manager):
        ...
        with pytest.raises(
            ValueError, match="Source list 'nonexistent' does not exist"
        ):
    ...
    def test_link_list_target_exists(self, manager, sample_source_list):
        ...
        with pytest.raises(
            ValueError, match="Target list 'target_list' already exists"
        ):
