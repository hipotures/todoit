...
class TestListAllEnhancements:
    ...
    def manager_with_failed_tasks(self, manager):
        ...
        # Create test lists
        list1 = manager.create_list("test_list_1", "Test List 1", ["Task 1", "Task 2", "Task 3"])
        ...
        if len(items_list1) >= 3:
            manager.update_item_status("test_list_1", items_list1[0].item_key, "completed")
            manager.update_item_status("test_list_1", items_list1[1].item_key, "failed")  # Set one task to failed
            manager.update_item_status("test_list_1", items_list1[2].item_key, "in_progress")
        
        if len(items_list2) >= 1:
            manager.update_item_status("test_list_2", items_list2[0].item_key, "failed")  # Another failed task
    ...
    @pytest.mark.asyncio
    async def test_mcp_list_all_includes_progress_stats(self, manager_with_failed_tasks):
        ...
        for list_data in lists:
            ...
            expected_fields = ["total", "completed", "in_progress", "pending", "failed", 
                             "completion_percentage", "blocked", "available"]
    ...
    def test_json_output_includes_failed_stats(self, mock_stdout, manager_with_failed_tasks):
