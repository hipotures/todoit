...
class TestHierarchyAccessPatterns:
    ...
    def test_subitem_access_with_parent_key(self, manager_with_hierarchy):
        ...
        level1 = manager_with_hierarchy.get_item(
            "test", "level1", parent_item_key="root"
        )
    ...
    def test_deep_subitem_access(self, manager_with_hierarchy):
        ...
        level2 = manager_with_hierarchy.get_item(
            "test", "level2", parent_item_key="level1"
        )
    ...
    def test_wrong_parent_key_fails(self, manager_with_hierarchy):
        ...
        result = manager_with_hierarchy.get_item(
            "test", "level2", parent_item_key="root"
        )
        assert result is None
    ...
    def test_update_operations_require_correct_parent_key(self, manager_with_hierarchy):
        ...
        manager_with_hierarchy.update_item_status(
            "test", "level2", "completed", parent_item_key="level1"
        )
        item = manager_with_hierarchy.get_item(
            "test", "level2", parent_item_key="level1"
        )
        ...
        with pytest.raises(ValueError, match="not found under parent"):
            manager_with_hierarchy.update_item_status(
                "test", "level2", "completed", parent_item_key="root"
            )

    def test_items_with_children_cannot_have_manual_status_change(
        self, manager_with_hierarchy
    ):
        ...
        with pytest.raises(ValueError, match="has subtasks"):
            manager_with_hierarchy.update_item_status(
                "test", "level1", "completed", parent_item_key="root"
            )
    ...
    def test_delete_operations_require_correct_parent_key(self, manager_with_hierarchy):
        ...
        # This should work
        success = manager_with_hierarchy.delete_item(
            "test", "extra", parent_item_key="root"
        )
