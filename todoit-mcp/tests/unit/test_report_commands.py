...
class TestReportCommands:
    ...
    def manager_with_failed_tasks(self, manager):
        ...
        list1 = manager.create_list("0001_project_alpha", "Project Alpha", ["Task 1", "Task 2", "Task 3"])
        list2 = manager.create_list("0023_beta_sprint", "Beta Sprint", ["Feature A", "Bug fix"])
        list3 = manager.create_list("simple_project", "Simple Project", ["Regular task"])
        ...
        if items1:
            manager.update_item_status("0001_project_alpha", items1[0].item_key, "failed")
            manager.update_item_status("0001_project_alpha", items1[1].item_key, "completed")
            # Add properties to failed item
            manager.set_item_property("0001_project_alpha", items1[0].item_key, "error_type", "timeout")
            manager.set_item_property("0001_project_alpha", items1[0].item_key, "retry_count", "3")
        
        if items2:
            manager.update_item_status("0023_beta_sprint", items2[0].item_key, "failed")
            manager.update_item_status("0023_beta_sprint", items2[1].item_key, "in_progress")
            # Add properties to failed item
            manager.set_item_property("0023_beta_sprint", items2[0].item_key, "severity", "high")
    ...
    def test_get_all_failed_items_with_regex_filter(self, manager_with_failed_tasks):
        ...
        filtered_items = manager_with_failed_tasks.get_all_failed_items(list_filter=r"^\d{4}_.*")
        ...
        specific_filter = manager_with_failed_tasks.get_all_failed_items(list_filter=r"^0001_.*")
    ...
    def test_get_all_failed_items_no_matches(self, manager_with_failed_tasks):
        ...
        no_matches = manager_with_failed_tasks.get_all_failed_items(list_filter=r"^nonexistent_.*")
    ...
    def test_get_all_failed_items_includes_properties(self, manager_with_failed_tasks):
        ...
        alpha_item = next(item for item in failed_items if item["list_key"] == "0001_project_alpha")
        ...
        beta_item = next(item for item in failed_items if item["list_key"] == "0023_beta_sprint")
    ...
    def test_get_all_failed_items_ignores_archived_lists(self, manager_with_failed_tasks):
    ...
    async def test_mcp_todo_report_errors_basic(self, manager_with_failed_tasks):
        ...
        with patch('interfaces.mcp_server.init_manager', return_value=manager_with_failed_tasks):
    ...
    @pytest.mark.asyncio 
    async def test_mcp_todo_report_errors_datetime_serialization(self, manager_with_failed_tasks):
    ...
    def test_cli_display_format_table(self, mock_stdout, manager_with_failed_tasks):
        ...
        for item_info in failed_items:
            props_str = ", ".join([f"{k}={v}" for k, v in item_info.get('properties', {}).items()])
            record = {
                "List": item_info['list_key'],
                "Item": item_info['item_key'],
                "Content": item_info['content'][:35],
                "Properties": props_str[:30] + "..." if len(props_str) > 30 else props_str
            }
            data.append(record)
    ...
    def test_regex_patterns_common_cases(self, manager_with_failed_tasks):
        ...
        for pattern, expected_count, description in patterns:
            filtered_items = manager_with_failed_tasks.get_all_failed_items(list_filter=pattern)
            assert len(filtered_items) == expected_count, f"Pattern {pattern} ({description}) should return {expected_count} items"
