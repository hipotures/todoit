...
class TestDynamicTagColors:
    ...
    def test_color_consistency_across_methods(self, manager):
        ...
        assert gamma_color_all == "blue"  # gamma is 3rd alphabetically (alpha, delta, gamma)
    ...
    def test_middle_tag_deletion_shift(self, manager):
        ...
        assert remaining_colors["epsilon"] == "blue"   # Shifted from yellow to blue (index 2)
        assert remaining_colors["gamma"] == "yellow"   # Shifted from orange to yellow (index 3)
    ...
    def test_list_specific_tag_color_consistency(self, manager):
        ...
        all_tags = manager.get_all_tags()
        shared_color_global = next(tag.color for tag in all_tags if tag.name == "shared_tag")
