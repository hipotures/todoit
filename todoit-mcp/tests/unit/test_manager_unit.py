...
class TestTodoManagerUnit:
    ...
    def test_get_next_pending_algorithm(self, manager_with_mock, mock_db):
        ...
        mock_parent = MagicMock(id=1, item_key="task1", status="in_progress", parent_item_id=None, position=1)
        mock_subtask1 = MagicMock(id=2, item_key="task2", status="pending", parent_item_id=1, position=1)
        mock_subtask2 = MagicMock(id=3, item_key="task3", status="pending", parent_item_id=1, position=2)
    ...
    def test_is_item_not_blocked_if_dependency_is_completed(self, manager_with_mock, mock_db):
        ...
        with patch.object(manager_with_mock, '_db_to_model', side_effect=lambda db_obj, model_class: db_obj):
            with pytest.raises(ValueError, match="Circular dependency detected"):
                manager_with_mock.add_item_dependency(
                    dependent_list="test", dependent_item="item_b",
                    required_list="test", required_item="item_a"
                )

    def test_get_next_pending_handles_orphaned_subtasks(self, manager_with_mock, mock_db):
    ...
    def test_move_to_subtask(self, manager_with_mock, mock_db):
        ...
        mock_db.update_item.assert_called_once_with(item_to_move.id, {"parent_item_id": new_parent.id})

    def test_add_subtask_to_nonexistent_parent_raises_error(self, manager_with_mock, mock_db):
        """Test that adding a subtask to a non-existent parent raises ValueError."""
        mock_db.get_item_by_key.return_value = None

        with pytest.raises(ValueError, match="Parent task 'nonexistent_parent' not found"):
    ...
    def test_add_dependency_to_nonexistent_item_raises_error(self, manager_with_mock, mock_db):
        """Test that adding a dependency to a non-existent item raises ValueError."""
        item_a = MagicMock(id=1)
        mock_db.get_item_by_key.side_effect = [item_a, None]

        with pytest.raises(ValueError, match="Required item 'nonexistent_item' not found"):
            manager_with_mock.add_item_dependency(
                dependent_list="test", dependent_item="item_a",
                required_list="test", required_item="nonexistent_item"
            )
    ...
    def test_delete_item_cascades_to_subtasks(self, manager_with_mock, mock_db):
        ...
        parent = MagicMock(id=1, item_key='parent_key')
