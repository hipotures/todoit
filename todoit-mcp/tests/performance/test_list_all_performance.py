...
class TestListAllPerformance:
    ...
    def test_bulk_progress_performance(self, large_manager):
        ...
        print(f"Estimated individual time for all lists: {individual_time * len(list_keys) / 10:.3f}s")
        
        # Bulk should complete in under 1 second for 100 lists
        assert bulk_time < 1.0, f"Bulk progress took {bulk_time:.3f}s, should be under 1.0s"
        ...
        for list_key in list_keys[:5]:
            ...
            assert abs(bulk_stats.completion_percentage - individual_stats.completion_percentage) < 0.1
    ...
    def test_database_bulk_operations_performance(self, large_manager):
        ...
        print(f"Database status counts time for {len(list_ids)} lists: {status_time:.3f}s")
        assert status_time < 0.1, f"Database status counts took {status_time:.3f}s, should be under 0.1s"
        ...
        assert tags_time < 0.1, f"Database tags took {tags_time:.3f}s, should be under 0.1s"
        ...
        for list_id, counts in status_counts.items():
            ...
            assert all(key in counts for key in ['pending', 'in_progress', 'completed', 'failed'])
    ...
    def test_list_all_overall_performance(self, large_manager):
        ...
        for todo_list in lists:
            ...
            record = {
                ...
                "Percentage": f"{progress.completion_percentage:.0f}%" if progress else "0%",
            }
            data.append(record)
        
        total_time = time.time() - start_time
        
        print(f"Total list_all simulation time for {len(lists)} lists: {total_time:.3f}s")
        print(f"Average time per list: {total_time / len(lists) * 1000:.1f}ms")
        
        # Should complete in under 2 seconds for 100 lists
        assert total_time < 2.0, f"List all simulation took {total_time:.3f}s, should be under 2.0s"
        ...
        for record in data[:5]:  # Check first 5
            assert all(key in record for key in ["ID", "Key", "Title", "Tags", "Pending", "Completed"])
    ...
    def test_performance_regression_detection(self, large_manager):
        ...
        assert avg_time < 1.0, f"Average time {avg_time:.3f}s indicates performance regression"
        assert max_time < 1.5, f"Max time {max_time:.3f}s indicates performance regression"
        
        # Times shouldn't vary too much (indicates caching/index issues)
        time_variation = max_time - min(times)
        assert time_variation < 0.5, f"Time variation {time_variation:.3f}s indicates inconsistent performance"
...
class TestBulkOperationsCorrectness:
    ...
    def test_bulk_progress_correctness(self, test_manager):
        ...
        for list_key in list_keys:
            ...
            assert abs(bulk.completion_percentage - individual.completion_percentage) < 0.1
