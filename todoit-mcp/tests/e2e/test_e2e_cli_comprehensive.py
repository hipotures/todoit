...
class TestE2EComprehensiveCLI:
    ...
    def test_complete_project_lifecycle(self, temp_db):
        ...
        result = runner.invoke(cli, [*db_arg, "list", "create", "--list", "backend", "--title", "Backend Development"])
        assert result.exit_code == 0
        assert "List Created" in result.output

        result = runner.invoke(cli, [*db_arg, "list", "create", "--list", "frontend", "--title", "Frontend Development"])
        assert result.exit_code == 0
        assert "List Created" in result.output

        result = runner.invoke(cli, [*db_arg, "list", "create", "--list", "testing", "--title", "Quality Assurance"])
        assert result.exit_code == 0
        assert "List Created" in result.output

        # Create and assign tags
        result = runner.invoke(cli, [*db_arg, "tag", "create", "--name", "urgent", "--color", "red"])
        assert result.exit_code == 0
        assert "Tag 'urgent' created" in result.output

        result = runner.invoke(cli, [*db_arg, "tag", "create", "--name", "enhancement", "--color", "blue"])
        assert result.exit_code == 0
        assert "Tag 'enhancement' created" in result.output

        result = runner.invoke(cli, [*db_arg, "list", "tag", "add", "--list", "backend", "--tag", "urgent"])
        assert result.exit_code == 0
        assert "Added tag 'urgent'" in result.output

        result = runner.invoke(cli, [*db_arg, "list", "tag", "add", "--list", "frontend", "--tag", "enhancement"])
        assert result.exit_code == 0
        assert "Added tag 'enhancement'" in result.output

        # Set project properties
        result = runner.invoke(cli, [*db_arg, "list", "property", "set", "--list", "backend", "--key", "deadline", "--value", "2025-09-01"])
        assert result.exit_code == 0
        assert "Set property 'deadline'" in result.output

        result = runner.invoke(cli, [*db_arg, "list", "property", "set", "--list", "backend", "--key", "lead_developer", "--value", "Alice"])
        assert result.exit_code == 0

        # Verify properties
        result = runner.invoke(cli, [*db_arg, "list", "property", "get", "--list", "backend", "--key", "deadline"])
        assert result.exit_code == 0
        ...
        # Add main tasks
        result = runner.invoke(cli, [*db_arg, "item", "add", "--list", "backend", "--item", "api", "--title", "REST API Development"])
        assert result.exit_code == 0
        assert "Added item 'api'" in result.output

        result = runner.invoke(cli, [*db_arg, "item", "add", "--list", "backend", "--item", "database", "--title", "Database Design"])
        assert result.exit_code == 0

        result = runner.invoke(cli, [*db_arg, "item", "add", "--list", "frontend", "--item", "ui", "--title", "User Interface"])
        assert result.exit_code == 0

        result = runner.invoke(cli, [*db_arg, "item", "add", "--list", "testing", "--item", "unit_tests", "--title", "Unit Testing"])
        assert result.exit_code == 0

        # Add subtasks
        result = runner.invoke(cli, [*db_arg, "item", "add", "--list", "backend", "--item", "api", "--subitem", "auth", "--title", "Authentication"])
        assert result.exit_code == 0
        assert "Added subitem 'auth'" in result.output

        result = runner.invoke(cli, [*db_arg, "item", "add", "--list", "backend", "--item", "api", "--subitem", "endpoints", "--title", "API Endpoints"])
        assert result.exit_code == 0

        result = runner.invoke(cli, [*db_arg, "item", "add", "--list", "backend", "--item", "database", "--subitem", "schema", "--title", "Database Schema"])
        assert result.exit_code == 0
        ...
        result = runner.invoke(cli, [*db_arg, "dep", "add", "--dependent", "frontend:ui", "--required", "backend:api"], input="y\n")
        assert result.exit_code == 0
        assert "Dependency added" in result.output

        result = runner.invoke(cli, [*db_arg, "dep", "add", "--dependent", "testing:unit_tests", "--required", "backend:database"], input="y\n")
        assert result.exit_code == 0
        assert "Dependency added" in result.output
        ...
        result = runner.invoke(cli, [*db_arg, "item", "status", "--list", "backend", "--item", "api", "--subitem", "auth", "--status", "in_progress"])
        assert result.exit_code == 0
        assert "Updated subitem 'auth' status" in result.output

        result = runner.invoke(cli, [*db_arg, "item", "status", "--list", "backend", "--item", "api", "--subitem", "auth", "--status", "completed"])
        assert result.exit_code == 0

        result = runner.invoke(cli, [*db_arg, "item", "status", "--list", "backend", "--item", "api", "--subitem", "endpoints", "--status", "completed"])
        assert result.exit_code == 0
        ...
        result = runner.invoke(cli, [*db_arg, "io", "export", "--list", "backend", "--file", export_file])
        assert result.exit_code == 0
        ...
        # Test JSON output
        result = runner.invoke(cli, [*db_arg, "list", "all"], env={"TODOIT_OUTPUT_FORMAT": "json"})
        assert result.exit_code == 0
        ...
        # Test YAML output
        result = runner.invoke(cli, [*db_arg, "list", "all"], env={"TODOIT_OUTPUT_FORMAT": "yaml"})
        assert result.exit_code == 0
        ...
        result = runner.invoke(cli, [*db_arg, "item", "status", "--list", "backend", "--item", "database", "--subitem", "schema", "--status", "completed"])
        assert result.exit_code == 0

        result = runner.invoke(cli, [*db_arg, "item", "status", "--list", "frontend", "--item", "ui", "--status", "completed"])
        assert result.exit_code == 0

        result = runner.invoke(cli, [*db_arg, "item", "status", "--list", "testing", "--item", "unit_tests", "--status", "completed"])
        assert result.exit_code == 0
        ...
        assert "Quality" in result.output and "Assurance" in result.output  # Not archived
        assert len(result.output.split('\n')) < 15  # Shorter output without archived lists
    ...
    def test_system_robustness(self, temp_db):
        ...
        result = runner.invoke(cli, [*db_arg, "list", "property", "get", "--list", "robust_test", "--key", "nonexistent_property"])
        assert result.exit_code == 0  # Should handle gracefully
        
        # Test creating multiple lists with similar names
        result = runner.invoke(cli, [*db_arg, "list", "create", "--list", "similar1", "--title", "Similar List 1"])
        assert result.exit_code == 0
        
        result = runner.invoke(cli, [*db_arg, "list", "create", "--list", "similar2", "--title", "Similar List 2"])
        assert result.exit_code == 0
    ...
    def test_emoji_mapping_in_outputs(self, temp_db):
        ...
        runner.invoke(cli, [*db_arg, "item", "add", "--list", "emoji_test", "--item", "task1", "--title", "Task 1"])
        runner.invoke(cli, [*db_arg, "item", "status", "--list", "emoji_test", "--item", "task1", "--status", "completed"])
