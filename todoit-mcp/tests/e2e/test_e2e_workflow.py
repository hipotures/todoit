...
class TestE2EWorkflow:
    ...
    def test_complete_project_workflow(self, temp_db):
        ...
        res_create_backend = runner.invoke(cli, [*db_arg, 'list', 'create', 'backend', '--title', 'Backend Development'])
        assert res_create_backend.exit_code == 0
        assert "List Created" in res_create_backend.output

        res_create_frontend = runner.invoke(cli, [*db_arg, 'list', 'create', 'frontend', '--title', 'Frontend Development'])
        ...
        runner.invoke(cli, [*db_arg, 'item', 'add-subtask', 'backend', 'api', 'auth', 'Authentication endpoints'])
        runner.invoke(cli, [*db_arg, 'item', 'add', 'frontend', 'ui', 'User Interface'])

        # STEP 3: Create a cross-list dependency
        res_dep_add = runner.invoke(cli, [*db_arg, 'dep', 'add', 'frontend:ui', 'requires', 'backend:api'], input='y\n')
        ...
        res_done_subtask = runner.invoke(cli, [*db_arg, 'item', 'status', 'backend', 'auth', '--status', 'completed'])
        assert res_done_subtask.exit_code == 0
        
        # When the last subtask is completed, the parent should auto-complete
        res_show_backend = runner.invoke(cli, [*db_arg, 'list', 'show', 'backend', '--tree'])
        ...
        res_next_frontend_unblocked = runner.invoke(cli, [*db_arg, 'item', 'next', 'frontend'])
