...
class TestSystemLimits:
    ...
    def test_maximum_lists_stress(self, temp_manager):
        ...
        random_list = temp_manager.get_list(f"list{random.randint(0, created_count-1):05d}")
    ...
    def test_deep_hierarchy_stress(self, temp_manager):
        ...
        try:
            for i in range(max_depth):
                ...
                temp_manager.add_subitem("deep", current_parent, child_key, f"Task at depth {i}")
    ...
    def test_wide_hierarchy_stress(self, temp_manager):
        ...
        try:
            for i in range(max_children):
                temp_manager.add_subitem("wide", "parent", f"child{i:04d}", f"Child task {i}")
                created_count += 1
    ...
    def test_maximum_properties_stress(self, temp_manager):
        ...
        try:
            for i in range(max_properties):
                temp_manager.set_item_property("props", "task", f"prop{i:04d}", f"value{i}")
                created_count += 1
    ...
    def test_maximum_dependencies_stress(self, temp_manager):
        ...
        max_dependencies = (num_lists - 1) * items_per_list * items_per_list  # Now 4*10*10 = 400
                ...
                for j in range(items_per_list):
                    for k in range(items_per_list):
                        ...
                        if created_count % 500 == 0:
                            elapsed = time.time() - start_time
                            print(f"Created {created_count} dependencies in {elapsed:.2f}s")
        
        except Exception as e:
            pytest.fail(f"System failed after creating {created_count} dependencies: {e}")
    ...
    def test_database_size_limits(self, temp_manager):
        ...
        try:
            for i in range(num_lists):
                ...
                for j in range(items_per_list):
                    ...
                    content = f"Database growth test item {j} with some content to increase DB size. " * 10
                    temp_manager.add_item(list_key, f"item{j:03d}", content)
                    total_items += 1
                    
                    # Add some properties
                    temp_manager.set_item_property(list_key, f"item{j:03d}", "priority", "normal")
                    temp_manager.set_item_property(list_key, f"item{j:03d}", "category", f"cat{j%5}")
        ...
        creation_time = time.time() - start_time
        print(f"Created large database with {total_items} items in {creation_time:.2f}s")
        ...
        try:
            for i in range(operations_count):
                ...
                if i % 4 == 0:
                    ...
                    temp_manager.update_item_status("rapid", f"task{i-1}", ItemStatus.IN_PROGRESS)
                elif i % 4 == 2:
                    temp_manager.set_item_property("rapid", f"task{i-2}", "priority", "high")
                else:
                    try:
    ...
    def test_memory_stress_with_cleanup(self, temp_manager):
        ...
        for cycle in range(cycles):
            ...
            for i in range(items_per_cycle):
                content = f"Memory stress test content for cycle {cycle} item {i}. " * 20
            ...
            if cycle % 10 == 0:
                print(f"Cycle {cycle}: Create={creation_time:.2f}s, Cleanup={cleanup_time:.2f}s, Deleted={deleted}")
