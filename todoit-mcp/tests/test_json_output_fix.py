...
from interfaces.cli_modules.display import _get_output_format, _render_table_view
...
class TestCLIJSONOutputFix:
    ...
    def test_cli_json_single_output(self, manager_with_test_list, monkeypatch, capsys):
        ...
        assert (
            len(json_outputs) == 1
        ), f"Expected 1 JSON output, got {len(json_outputs)}"
    ...
    def test_cli_table_mode_unchanged(
        self, manager_with_test_list, monkeypatch, capsys
    ):
...
class TestMCPEnhancedGetList:
    ...
    async def test_mcp_get_list_with_all_data(self, manager_with_test_list):
        ...
        completed_items = [
            item for item in result["items"]["data"] if item["status"] == "completed"
        ]
...
class TestBackwardCompatibility:
    ...
    def test_cli_other_formats_unchanged(
        self, manager_with_test_list, monkeypatch, capsys
    ):
