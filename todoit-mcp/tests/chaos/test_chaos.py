...
class TestChaosEngineering:
    ...
    def test_random_operation_failures(self, temp_manager):
            ...
            try:
                if operation == "add_item":
                    ...
                    temp_manager.set_item_property(
                        "chaos", f"task{task_id}", f"prop{i}", f"value{i}"
                    )
                elif operation == "get_property":
                    task_id = random.randint(0, 9)
                    try:
                        temp_manager.get_item_property(
                            "chaos", f"task{task_id}", "priority"
                        )
                    except ValueError:
                    ...
                    temp_manager.add_subitem(
                        "chaos", f"task{task_id}", f"sub{i}", f"Subtask {i}"
                    )
        ...
        print(
            f"Chaos test: {operations_successful}/{operations_attempted} operations succeeded, {failures_encountered} failures"
        )
    ...
    def test_memory_pressure_simulation(self, temp_manager):
        ...
        try:
            ...
            print(
                f"System handled memory pressure gracefully after {created_items} items: {e}"
            )
    ...
    def test_concurrent_access_chaos(self, temp_manager):
            ...
            try:
                if op_type == "add":
                    temp_manager.add_item(
                        "concurrent", f"item{item_id}", f"Item {item_id}"
                    )
                elif op_type == "read":
                    try:
                    ...
                    try:
                        temp_manager.update_item_status(
                            "concurrent", f"item{item_id}", ItemStatus.IN_PROGRESS
                        )
                    except ValueError:
                        pass  # Item might not exist yet
                elif op_type == "delete":
                    try:
                        ...
                        temp_manager.update_item_status(
                            "concurrent", f"item{item_id}", ItemStatus.COMPLETED
                        )
                    except ValueError:
                        pass  # Item might not exist yet
        ...
        print(
            f"Concurrent chaos: {successful_ops} successful, {failed_ops} failed operations"
        )
    ...
    def test_invalid_data_injection(self, temp_manager):
        ...
        for invalid_key, description in invalid_inputs:
            try:
                ...
                temp_manager.add_item(
                    "invalid", invalid_key, f"Content for {description}"
                )
        ...
        assert (
            successful_creates + handled_gracefully == len(invalid_inputs) - 1
        )  # -1 for None
    ...
    def test_resource_exhaustion_simulation(self, temp_manager):
        ...
        try:
            ...
            for i in range(1000):  # Try to create many properties
                temp_manager.set_item_property(
                    "resource", "test_item", f"prop{i:04d}", f"value{i}"
                )
    ...
    def test_transaction_chaos(self, temp_manager):
        ...
        for i in range(20):
            try:
                ...
                for j in range(5):
                    temp_manager.add_subitem(
                        "transaction", parent_key, f"child{i}_{j}", f"Child {i}-{j}"
                    )

                # Add properties (more database operations)
                temp_manager.set_item_property(
                    "transaction", parent_key, "priority", "high"
                )
                temp_manager.set_item_property(
                    "transaction", parent_key, "category", f"cat{i}"
                )
        ...
        print(
            f"Transaction chaos: {operations_completed} operations completed, {parent_count} parents, {child_count} children"
        )
    ...
    def test_random_system_state_corruption(self, temp_manager):
        ...
        for _ in range(20):
            ...
            try:
                temp_manager.update_item_status(
                    "state", random_item.item_key, random_status
                )
            except Exception:
...
class TestChaosRecovery:
    ...
    def test_recovery_after_unexpected_shutdown(self, temp_manager_path):
        ...
        for i in range(10):
            ...
            if i % 2 == 0:
                manager1.update_item_status(
                    "recovery", f"task{i}", ItemStatus.IN_PROGRESS
                )
        ...
        while stress_level < max_stress:
            try:
                ...
                for i in range(batch_size):
                    ...
                    content = f"Stress test item {stress_level}-{i} with content. " * (
                        stress_level // 100 + 1
                    )
                    ...
                    if stress_level % 50 == 0:
                        manager.set_item_property(
                            "stress", item_key, "stress_level", str(stress_level)
                        )
