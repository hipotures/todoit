...
class TestLimits:
    ...
    def test_very_long_task_content(self, manager):
        ...
        try:
            ...
            assert any(keyword in error_msg for keyword in ['string_too_long', '1000 characters', 'validation'])
    ...
    def test_deep_subtask_hierarchy(self, manager):
        ...
        for depth in range(1, max_depth + 1):
            ...
            manager.add_subtask("deep_list", parent_key, subtask_key, f"Subtask at level {depth}")
    ...
    def test_many_subtasks_per_parent(self, manager):
        ...
        manager.update_item_status("wide_list", f"child_{num_subtasks - 1}", "completed")
    ...
    def test_empty_and_whitespace_values(self, manager):
        ...
        for i, value in enumerate(edge_case_values):
            try:
                ...
                error_msg = str(e).lower()
                assert any(keyword in error_msg for keyword in ['empty', 'required', 'validation', 'exists'])
        ...
        for i, value in enumerate(edge_case_values):
            try:
                ...
                # Should handle gracefully
                error_msg = str(e).lower()
                assert any(keyword in error_msg for keyword in ['empty', 'content', 'validation'])
        ...
        try:
            ...
            for i in range(50):  # Reasonable number for testing
                try:
                    ...
                    error_msg = str(e).lower()
                    assert any(keyword in error_msg for keyword in ['size', 'space', 'limit', 'full', 'string_too_long', '1000 characters'])
                    break
