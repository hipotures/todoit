...
class TodoListBase(BaseModel):
    ...
    def validate_list_key(cls, v):
        ...
        if not v.replace("_", "").replace("-", "").replace(".", "").isalnum():
            raise ValueError(
                "list_key must contain only alphanumeric characters, underscores, hyphens, and dots"
            )
        return v
...
class TodoItemBase(BaseModel):
    ...
    def validate_item_key(cls, v):
        ...
        if not v.replace("_", "").replace("-", "").replace(".", "").isalnum():
            raise ValueError(
                "item_key must contain only alphanumeric characters, underscores, hyphens, and dots"
            )
        return v
...
class ListPropertyBase(BaseModel):
    ...
    def validate_property_key(cls, v):
        ...
        if not re.match(r"^[a-zA-Z0-9_\-.:]+$", v):
            raise ValueError(
                "property_key must contain only alphanumeric characters, underscores, hyphens, dots, and colons"
            )
    ...
    def validate_property_value(cls, v):
        ...
        for pattern in dangerous_patterns:
            if pattern in v_lower:
                raise ValueError(
                    f"property_value contains potentially dangerous content: {pattern}"
                )
        ...
        if "<" in v and ">" in v:
            ...
            if re.search(r"<(?!/?(?:b|i|u|em|strong|br|p)/?)[^>]*>", v, re.IGNORECASE):
                raise ValueError(
                    "property_value contains potentially dangerous HTML tags"
                )
...
class ItemDependencyBase(BaseModel):
    ...
    dependent_item_id: int = Field(
        ..., gt=0, description="ID of item that depends on another"
    )
...
class ItemDependencyCreate(ItemDependencyBase):
    ...
    def validate_not_self_dependency(cls, v, info):
        ...
        if (
            info.data
            and "dependent_item_id" in info.data
            and v == info.data["dependent_item_id"]
        ):
