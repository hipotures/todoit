...
class SecureFileHandler:
    ...
    def validate_file_path(
        file_path: str, allowed_base_dirs: Optional[Set[str]] = None
    ) -> str:
        """
        ...
        for forbidden in [
            ...
            if forbidden in file_path:
                raise SecurityError(
                    f"Forbidden character detected in path: {repr(forbidden)}"
                )
        ...
        try:
            ...
            for part in path_obj.parts:
                ...
                for forbidden in SecureFileHandler.FORBIDDEN_COMPONENTS:
                    if forbidden in part:
                        raise SecurityError(
                            f"Forbidden character '{forbidden}' in path component: {part}"
                        )
            ...
            if allowed_base_dirs:
                ...
                if not any(
                    path_str.startswith(base_dir) for base_dir in allowed_base_dirs
                ):
                    raise SecurityError(
                        f"Path not within allowed directories: {path_str}"
                    )
    ...
    def validate_file_extension(file_path: str) -> None:
        ...
        if extension not in SecureFileHandler.ALLOWED_EXTENSIONS:
            ...
            raise SecurityError(
                f"File extension '{extension}' not allowed. Allowed: {allowed}"
            )
    ...
    def secure_file_write(
        file_path: str, content: str, allowed_base_dirs: Optional[Set[str]] = None
    ) -> None:
        """
